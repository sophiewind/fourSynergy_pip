...
Usage: bamToReTab.pl [options] LENGTH_A MAX_SIZE MIN_SIZE BAM_File.bam 3C_RE_SITES.txt 4C_RE_SITES.txt LI_RE_SITES.txt > OUTPUT.tab
...
-c  CHR_STRING
    By default, bamToReTab assumes that each chromosome name is formated like
...
open(SAM_FILE, "samtools view -F 4 -q $minQualScore $bowtieFile |") 
    || die("Could not open $bowtieFile $!\n");
#open(SAM_FILE, "<".$bowtieFile) 
my %bowtieBins;
my $counter = 0;
