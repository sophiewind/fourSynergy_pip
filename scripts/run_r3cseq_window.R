...
#library(BSgenome.Mmusculus.UCSC.mm9.masked)  # TODO  was ist mit human
...
window.size <- commandArgs(trailingOnly = T)[3]
...
if(!is.null(config$control)){
  ...
  getBatchReadCountPerWindow(my3_obj, windowSize = as.numeric(window.size))
  ...
  setwd('../../..')
  getwd()
  pdf(paste0(OUT.PATH, 'interactions_near_VP_r3Seq_', window.size, '.pdf'))
  try(plotInteractionsNearViewpoint (my3_obj))
  dev.off()
  
  pdf(paste0(OUT.PATH, 'interactions_chr', config$VPchr,'_r3Seq_', window.size, '.pdf'))
  try(plotInteractionsPerChromosome(my3_obj, paste0('chr', config$VPchr)))
  dev.off()
  
  pdf(paste0(OUT.PATH, 'interactions_overview_interactions_r3Seq_', window.size, '.pdf'))
  try(plotOverviewInteractions(my3_obj))
  dev.off()
  #plotDomainogramNearViewpoint(my3_obj)
  
  
  try(saveRDS(my3_obj, paste0(OUT.PATH, 'r3Cseq_', window.size, '_obj.rds')))
  ...
  tryCatch({
    for (bam in bams){
      ...
      try(getReadCountPerWindow(my3_obj, windowSize = as.numeric(window.size)))
      ...
      pdf(paste0(OUT.PATH, gsub('.bam', '', (gsub('\"', "", deparse(bam, control = "all")))),
                 '_interactions_near_VP_r3Seq_', window.size, '.pdf'))
      try(plotInteractionsNearViewpoint (my3_obj))
      dev.off()
      
      pdf(paste0(OUT.PATH, gsub('.bam', '', (gsub('\"', "", deparse(bam, control = "all")))), 'interactions_chr', config$VPchr,'_r3Seq_', window.size, '.pdf'))
      try(plotInteractionsPerChromosome(my3_obj, paste0('chr', config$VPchr)))
      dev.off()
      
      pdf(paste0(OUT.PATH,gsub('.bam', '', (gsub('\"', "", deparse(bam, control = "all")))), 'interactions_overview_interactions_r3Seq_', window.size, '.pdf'))
      try(plotOverviewInteractions(my3_obj))
      dev.off()
      #plotDomainogramNearViewpoint(my3_obj)
      
      message(paste0(OUT.PATH, gsub('.bam', '', basename(bam)), '_r3Seq_', window.size, '_obj.rds'))
      #try(
      saveRDS(my3_obj, paste0(OUT.PATH, gsub('.bam', '', basename(bam)), '_r3Cseq_', window.size, '_obj.rds'))
      #)
     setwd(bam.path) 
    }
